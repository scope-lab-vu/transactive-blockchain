...
class Contract:
    ...
    # def call_func(self, from_account, getReceipt, price, name, aix, oid, ijoid, *args):
        ...
        self.helper.logEvent(now=datetime.datetime.now(), event=topic['name'], aix, oid, ijoid, value=1)
        ...
        for item in log:
            if self.address == item['address']:
                if item['topics'][0] in self.topics:
                    ...
                    self.helper.logEvent(now=datetime.datetime.now(), event=topic['name'], value=0)
