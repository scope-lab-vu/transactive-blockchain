...
To tackle these challenges, decentralized IoT solutions are emerging, which may arrange local communities into transactive microgrids.
...
On the one hand, consumers' personal information must be protected from their trade partners and \Abhishek{To what extent is this different that the information already known by the system operator? Is this more related to the ``real-time'' nature of transactive information? Rather than aggregative total information which is available at the end of billing cycle today.} \Aron{Do we need to detail this in the abstract?} the system operator.
...
Most of our security requirements follow readily from 
\begin{compactitem}
\item the non-malleability of transactions in the distributed ledger 
\item the signature based validation of transactions
\item tamper-resistance of smart meters.
\end{compactitem}

\subsection{Safety}

First, we can easily see that the net amount of energy sold by a prosumer for each timestep is at most $\field{MAXEPA}_i$.
The amount of energy sold by the prosumer for a timetep is less than or equal to the amount of EPA obtained for that timestep.
The prosumer can withdraw $\field{MAXEPA}_i$ amount of EPA from its smart meter.
However, to obtain any amount of further EPA, the prosumer has to buy energy from another prosumer, which would decrease its net energy sold by the same amount.
Similarly, the net amount of energy sold is at most $\field{MAXECA}_i$.

Second, we can see that the amount of bids posted at the same time by prosumer $i$ for timestep $t$ is at most $\field{MAXEPA}_i + \field{MAXECA}_i$.
