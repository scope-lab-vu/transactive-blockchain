...
\change{This section describes a basic system model of transactive IoT
microgrids and formulates security, safety, and privacy requirements.}{The microgrid is a collection of prosumers (residential nodes) that are arranged within the same distribution feeder that supports exchange of power between them. Prosumer nodes typically include a smart inverter and a smart meter that controls the flow of power into and out of the prosumer. Additionally, the microgrid contains a set of 
protection nodes that are  responsible for isolating faults on the feeder. Finally, a set of distribution system operator (DSO) controlled switching nodes control the connection of the microgrid to the rest of the distribution system. The Distribution Systems Operator (DSO) is responsible for regulating the net electric power into and out of the microgrid. Starting, from this model we describe the transactive microgrid model in the next section.}
...
Figure~\ref{fig:softwareArchitecture} shows the high-level architecture of 
\change{our solution.  In this figure, components marked red are introduced to
provide privacy in a safe and secure manner, whereas components marked
in blue are typical elements of a decentralized transactive microgrid.}{a decentralized transactive microgrid with PETra. 
Components marked in blue are basic elements of the decentralized transactive microgrid,
while components marked in red are added (or extended) by PETra.}

\begin{figure}[h!]
\center
\begin{tikzpicture}[x=8cm, y=0.9cm, font=\small,
