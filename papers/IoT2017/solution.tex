...
and security.
\Doug{Abhishek, could this figure appear earlier, i.e., in the previous section, as per my comment three?}
Figure~\ref{fig:softwareArchitecture} shows the architecture of 
\Doug{Abhishek, can we give ``our solution'' a name so that we can refer to it by name throughout the paper?}
...
Note that to prevent de-anonymization, a prosumer should use multiple addresses and multiple rounds of mixing, which we have omitted from the figure for ease of presentation.}
\label{fig:sellFlow}
\end{figure*}
...
transfers the production asset to an \emph{anonymous address} \circled{3},
%\Aron{We should probably insert a good definition here for readers who are unfamiliar with blockchain transactions.}
...
% knows the private key of the address (typically, the one who generated the address).}
...
post the energy ask \circled{4}, which contains an anonymous
...
\Doug{Abhishek, it seems like we might need a transition here since the previous paragraph seemed to wrap up the ``Overview of the Trading Process'' subsection above.  It's not clear how the rest of this text ties in with that discussion.  Perhaps we could use subsubsections to make these parts more distinct?}
\Aron{It did not wrap up. We have only discussed the process from the seller's perspective.}
