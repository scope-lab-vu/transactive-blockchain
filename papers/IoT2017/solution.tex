...
and security.
% Abhishek, could this figure appear earlier, i.e., in the previous
% section, as per my comment three?
Figure~\ref{fig:softwareArchitecture} shows the architecture of 
% Abhishek, can we give ``our solution'' a name so that we can refer
% to it by name throughout the paper?
...
the prosumer using a \emph{smart meter transaction} \circled{1}, which
...
using an \emph{energy and financial transaction} \circled{2}, which is
...
A prosumer can now engage in energy trading anonymously.  To find a
...
% Abhishek, it seems like we might need a transition here since the
% previous paragraph seemed to wrap up the ``Overview of the Trading
% Process'' subsection above.  It's not clear how the rest of this
% text ties in with that discussion.  Perhaps we could use
% subsubsections to make these parts more distinct? 
